      ...
      {data.data.length < 1 ? (
        ...
        <Card className="flex h-full w-1/2 flex-col">
          <CardHeader className="w-full rounded-t-lg border-b bg-sidebar">
            ...
            <CardDescription>Manage your meetings and schedule here.</CardDescription>
          </CardHeader>
            ...
            {data.data.map((event) => (
              ...
              >
                ...
                <div className="col-span-1 flex h-full w-full flex-col items-start justify-start">
                  <span className="w-full text-left text-muted-foreground text-sm">
                    {/* @ts-ignore */}
                  ...
                  <div className="mt-auto flex w-full items-center justify-start gap-2.5 pt-2">
                    ...
                    <Link
                      ...
                      className="flex-1 text-left text-primary text-sm hover:underline hover:underline-offset-4"
                    >
                      Join Meeting
                    </Link>
                  </div>
                ...
                <div className="col-span-1 flex h-full w-full flex-col items-start justify-start gap-2">
                  <span className="w-full text-left font-medium text-sm">{event.title}</span>
                  <div className="flex w-full flex-wrap items-center justify-start gap-2.5">
                    <span className="rounded-full bg-primary/20 px-3 py-1 text-xs text-yellow-600">You</span>
                    {event.participants.length > 0 &&
                      event.participants.map((participant, idx) => (
                        <span key={idx} className="rounded-full bg-primary/20 px-3 py-1 text-xs text-yellow-600">
                          {participant?.email}
                        </span>
                      ))}
                  </div>
