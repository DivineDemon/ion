...
import { postAppointmentSchema, updateAppointmentSchema } from "@/lib/validators";

import { createTRPCRouter, privateProcedure } from "../trpc";

export const eventRouter = createTRPCRouter({
  createEventType: privateProcedure.input(postAppointmentSchema).mutation(async ({ ctx, input }) => {
  ...
  updateEventType: privateProcedure.input(updateAppointmentSchema).mutation(async ({ ctx, input }) => {
